...
const ebtAgentSkills = createProject({
  ...
  currentlyContributing: true,
  shortDescription:
    "A coding-agent stack for Walking Skeleton Development — ten skills, four subagents, and a pre-commit hook that keep the running system as the source of truth",
  type: "personal",
  links: {
  ...
  keyFeatures: [
    "ebt-mode orchestrator drives shape → build sessions end-to-end with worker, rivals, coherence, and quality-gate agents",
    "Script-backed skeleton-walker and convention-scan skills replace eyeballing with structured JSON reports",
    "next-thickening artifact mechanically refuses anything but the walking skeleton as the first unit of work on greenfield projects",
    "validate-commit.py pre-commit hook blocks commits that regress the skeleton, use degenerate messages, or add unwarranted test files",
    "Ships as a single bundled Claude Code plugin via marketplace.json with namespaced skills and subagents",
    "Per-shape walking-skeleton playbook covering CLI, web service, library, and multi-agent project shapes",
  ],
  contributions: [
    "Designed v2 around Walking Skeleton Development after a v1 retrospective showed artifact-centric specs drifted from the running product",
    "Built ten skills covering orchestration, investigation, runnability checks, convention scanning, diff review, and domain modeling",
    "Authored four Claude subagents (worker, incorrectness-rival, system-coherence, code-quality-gate) with mirrored Codex variants",
    "Implemented mechanical enforcement via Python pre-commit hook integrated with Claude Code's PreToolUse hook contract",
    "Preserved v0 (XP-referenced, 25 skills) and v1 (adaptive spec-driven, 11 skills + 7 agents) iterations alongside v2 for traceability",
  ],
  metrics: [
    "10 skills + 4 subagents + 1 pre-commit hook in the v2 stack",
    "3 stack iterations preserved (v0, v1, v2) with retrospective and build-plan documents",
    "Cross-platform support for Claude Code and Codex CLI",
  ],
  tags: [
    "agent-skills",
    "walking-skeleton",
